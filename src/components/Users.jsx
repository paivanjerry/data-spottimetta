...
class Users extends Component {
    ...
    return (
      <div>
        ...
        <DataTable
          ...
          leftTitle="Nimimerkki"
          rightTitle="Lisättyjä spotteja"
        />
      </div>
    );
  }
  getTableData() {
    ...
    let avgNoOldMap =
      (spotSum - this.props.topList[0][1]) / (this.props.topList.length - 1);
    avgNoOldMap = (Math.round(100 * avgNoOldMap) / 100).toFixed(2);

    let avgNoDeveloper =
      (spotSum - this.props.topList[0][1] - this.props.topList[1][1]) /
      (this.props.topList.length - 2);
    avgNoDeveloper = (Math.round(100 * avgNoDeveloper) / 100).toFixed(2);

    let multispotPropability = (
      Math.round((100 * 100 * multipleSpots) / (multipleSpots + onlyOneSpot)) /
      100
    ).toFixed(2);
    ...
    finalData.push([
      "Sama ilman mitään sovelluksen tekijän spotteja",
      avgNoDeveloper
    ]);
    finalData.push([
